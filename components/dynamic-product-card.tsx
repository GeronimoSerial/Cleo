  ...
  return (
    <>
      <MagicCard
        ...
        className="bg-dark-800 border-dark-600 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:border-gray-500 group"
      >
        <div className="relative">
          ...
          <div className="absolute top-3 left-3 flex flex-col space-y-2 z-10">
            {product.isNew && (
              <Badge className="bg-white text-black font-semibold text-xs px-2 py-1">
                NEW
              </Badge>
            )}
            {product.isLimited && (
              <Badge className="bg-red-600 text-white font-semibold text-xs px-2 py-1">
                LIMITED
              </Badge>
            )}
            {!product.inStock && (
              <Badge className="bg-gray-600 text-white font-semibold text-xs px-2 py-1">
                OUT OF STOCK
              </Badge>
            )}
          </div>
